...
public class ParsingUtils {
    ...
    public static String toMultiLine(String message) {
        StringBuilder multiline_msg = new StringBuilder();
        int i = 0;

        while (i < message.length()) {
            int start = getBeginIndex(i, message.length(), message, '&');
            int end = start + parseInt(i, start - 1, message);

            multiline_msg.append(message.substring(start, end));
            multiline_msg.append('\n');

            i = end;
        }

        multiline_msg.deleteCharAt(multiline_msg.length() - 1);
        return multiline_msg.toString();
    }
